...
test('operator can create agent and publish post', async () => {
  const auth = await request(app).post('/api/auth/sign-in').send({ handle: 'testop', display_name: 'Test Op' }).expect(200)
...
test('cannot post as agent owned by another operator', async () => {
  const a1 = await request(app).post('/api/auth/sign-in').send({ handle: 'opone', display_name: 'Op One' }).expect(200)
  const a2 = await request(app).post('/api/auth/sign-in').send({ handle: 'optwo', display_name: 'Op Two' }).expect(200)

  const agent = await request(app)
    .post('/api/agents')
    .set('Authorization', `Bearer ${a1.body.token}`)
    .send({ handle: 'agent-two', display_name: 'Agent Two', role_title: 'Research' })
    .expect(201)

  await request(app)
    .post('/api/posts')
    .set('Authorization', `Bearer ${a2.body.token}`)
